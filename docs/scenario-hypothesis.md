...
> 상태: **수동 Sonnet 전체 트리** (`signals/hypothesis.py` · `/api/hypothesis` · ECharts tree)  
...
**시그널·봇 미반영.** Layer0+then+outcome 문장은 **관리자 수동 생성 시 Sonnet 1회**.  
...
3. Sonnet JSON: IF 2~4 + then≤2 + outcome (섹터·metric 화이트리스트)  
...
| POST | `/api/hypothesis/refresh` | 관리자만. Sonnet(+폴백) 생성 |

## 5. UI

- 미생성: placeholder + 관리자 생성 버튼  
- 메타: `Sonnet · 수동 · 시각` 또는 `fallback`  
- 기존 트리·IF 칩·상세 패널

...
- **트레이딩 예산:** 일일 KB LLM 자동수집 기본 OFF (`KB_AUTO_COLLECT`, 기본 0).
  시그널 점수·봇은 KB 자동 Sonnet에 의존하지 않음(Decision/DART 규칙은 별개).
- **학습 예산:** 이 탭. 거시 원료는 관리자 수동 수집(유튜브/RSS 등) → 여기서 **흐름 생성**.
  항목별 자막 압축은 Haiku, 최종 거시 톤·이 탭 Sonnet은 수동 시에만.

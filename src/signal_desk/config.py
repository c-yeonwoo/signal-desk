...
def llm_daily_budget_usd() -> float:
    ...
    2026-08-07: 1.0 → **5.0**. 기본 1.0은 로컬 실측(30일 누적 $1.11)에 맞춘 값이었는데,
    프로덕션 실제 소비는 **30일 $64.15**(일평균 $2.14 · 최근 7일 일평균 $3.08)였다.
    월 상한만 올리면 **일 상한에서 매일 막힌다** — 두 게이트를 같이 봐야 한다.
    5.0은 최근 7일 평균의 1.6배로, 평상시를 막지 않으면서 폭주(대화 루프·재시도 폭발)는
    하루 단위로 끊는다.
    """
    return _env_float("LLM_DAILY_BUDGET_USD", 5.0)


def llm_monthly_budget_usd() -> float:
    """월 지출 상한(USD). 일일 상한을 매일 다 써도 30배가 되지 않게 위에 하나 더 둔다.

    2026-08-07: 10.0 → **100.0**(사용자 지시). 프로덕션이 30일 $64.15로 상한의 6.4배를
    쓰고 있어 게이트가 계속 차단 중이었다. 일 5.0 × 30 = 150 이므로 월 100이 위에서 한 번 더
    끊는다 — 두 상한의 관계(월 < 일×30)를 유지한다.

    **이 값은 "얼마 쓰는지 안다"가 아니다.** 기능별 귀속(#355)이 쌓이면 무엇이 27,256회를
    냈는지 보이고, 그때 상한을 다시 정하거나 그 기능의 주기를 줄이는 것이 맞다.
    """
    return _env_float("LLM_MONTHLY_BUDGET_USD", 100.0)

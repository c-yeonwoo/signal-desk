...
METHODS = [
    ...
    {"key": "valuation_xs", "name": "밸류 횡단면 백분위", "category": "factor", "status": "active",
     ...
     "addresses": "저평가 매력", "evidence": "Value 팩터(Barra)", "risk": "고평가주 함정",
     "validate": "팩터 IC(valuation)", "note": "섹터 중립화 적용됨(v1) → sector_neutral 참조"},
    ...
    {"key": "crash_gate", "name": "단기 급락 게이트", "category": "gate", "status": "active",
     "idea": "당일·이틀 급락 시 신규 매수·분위 승격 차단", "formula": "1일≤−8% 또는 2일≤−12% → HOLD+gate_blocked",
     "addresses": "모멘텀/MACD가 못 보는 폭락 직후 우선매수", "evidence": "gap-down / short-term crash filter",
     "risk": "급락 당일 저점 매수 기회 놓침", "validate": "게이트 on/off 익일·5일 수익",
     "note": "rank 승격도 막음; 매도 신호는 유지"},
    {"key": "kb_event_veto", "name": "KB 악재 veto", "category": "gate", "status": "active",
     "idea": "Decision buy_blocked 시 매수권 하드 강등(급락 게이트와 동일)",
     "formula": "confirmed+eligible critical/serious → BUY→HOLD + gate_blocked + hold_tag 악재",
     "addresses": "매수 pill과 악재 경고 공존 방지", "evidence": "이벤트 리스크 veto",
     "risk": "오탐 시 매수권 과소", "validate": "veto 후 사후수익",
     "note": "점수 미반영; 장중 전이는 episode_state"},
    {"key": "execution_gate", "name": "실행 품질 게이트(추격·선반영)", "category": "gate", "status": "active",
     "idea": "발동 후 급등(late)·호재 선반영 의심이면 신규 매수 보류",
     "formula": "entry.quality=late 또는 priced_in.flag → HOLD+gate_blocked",
     "addresses": "고점 추격·이미 반영된 호재 매수", "evidence": "execution quality / priced-in filter",
     "risk": "강한 추세 지속 시 기회 비용", "validate": "게이트 on/off 5·20일 수익",
     "note": "점수 유지; UI·봇 공통 apply_from_store"},
    ...
    # ---------- candidate: 로드맵/후보 ----------
    {"key": "sector_neutral", "name": "섹터 중립화", "category": "structure", "status": "active",
     "idea": "팩터를 섹터 내 상대값으로 표준화", "formula": "섹터 내 percentile/z-score(작은 섹터는 유니버스 fallback)",
     "addresses": "섹터 편향 제거(반도체 고PER·고모멘텀 왜곡)", "evidence": "Barra USE4; MSCI factor investing",
     "risk": "섹터 표본 작으면 불안정·섹터오분류 전파", "validate": "트래커 팩터 IC 전후 비교",
     "note": "v1 밸류·목표가 + v2 모멘텀/수급 섹터 percentile 관측(점수 combine 전). 공매도는 추후"},
    ...
    {"key": "estimate_revision", "name": "추정치 리비전", "category": "factor", "status": "active",
    ...
    {"key": "multi_horizon", "name": "다중 horizon 신호 수명", "category": "structure", "status": "active",
     ...
     "validate": "트래커가 이미 5/20/60일 측정", "note": "관측 라벨(단기/스윙/포지션). kind·점수 불변"},
    {"key": "crowding_risk_lite", "name": "편중 경고(lite 리스크모델)", "category": "risk", "status": "active",
     "idea": "섹터·팩터 쏠림 감지", "formula": "매수권 섹터 집중도 ≥40% & n≥3 → warn",
     "addresses": "crowded trade 리스크", "evidence": "2025 퀀트펀드 손실(요인상관·crowding)",
     "risk": "전체 Barra는 과함(경고 수준만)", "validate": "쏠림 구간 드로다운",
     "note": "시그널 리스트·브리핑·data-health 경고. 점수/kind 불변"},
    {"key": "vol_adjusted_sizing", "name": "변동성 조정 사이징", "category": "sizing", "status": "active",
     "idea": "신호강도×역변동성 비중", "formula": "alloc × clamp(med_vol/vol, 0.5~1.5)",
     "addresses": "리스크 조정 수익", "evidence": "risk parity/vol targeting", "risk": "변동성 추정 오차",
     "validate": "리밸런싱 성과 A/B", "note": "봇 매수 alloc에만 부착"},
